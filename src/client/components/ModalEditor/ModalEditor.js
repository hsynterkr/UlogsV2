...
import ModalEditorFooter from './ModalEditorFooter';
...
class ModalEditor extends React.Component {
    static propTypes = {
      ...
      getQuickPostData = () => {
        ...
        data.parentPermlink = this.state.tags[1].length > 0 ? this.state.tags[1] : 'ulog';
      ...
      render() {
        const { imageUploading, currentImages, inputMinRows } = this.state;
        const { user, postCreationLoading, intl, certified } = this.props;
          
        const certDiv = (
          <div style={{padding: '5px'}}>
            <FormattedMessage  style={{padding: '5px'}} id="certmessage" className="certParagraph" 
              defaultMessage="Please note that while you will be able to successfully 
              post to steem and of course have your post appear on ulogs.org (as a post), 
              your post will not be displayed on our 'Ulog-Stories' display-column as this 
              column is reserved for 'certified uloggers' only. However, every(any) one can 
              become 'certified'. We invite you to contact us!" />
          </div>
        );
